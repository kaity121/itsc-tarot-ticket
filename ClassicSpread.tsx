...
import { RefreshCw, Layers, CheckCircle2, Share2 } from 'lucide-react';
...
export const ClassicSpread: React.FC<ClassicSpreadProps> = ({ onShare, language = 'en' }) => {
  ...
  const requestInterpretation = async (drawnCards: DrawnTarotCard[]) => {
    setIsLoadingAI(true);
    setReadingText(null);
    const cardNames = drawnCards.map(c => (language === 'vi' && c.nameVi ? c.nameVi : c.name));
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center py-4 sm:py-6 px-3 sm:px-4">
      ...
      {/* Mystical Loading Skeleton when waiting for AI */}
      {activeStep === 'revealed' && isLoadingAI && (
      ...
      {/* Reading Result View when readingText is available */}
      {activeStep === 'revealed' && !isLoadingAI && readingText && (
        <ReadingResultView
          reading={readingText}
          question={question}
          language={language}
